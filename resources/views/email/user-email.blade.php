...
<body>
    ...
    <div class="col-12 bg-light p-4 text-center" style="font-size: 1rem;">
        <p>Estimado/a {{$details->name}}, gracias por ponerte en contacto con nosotros tu peticion ha sido registrada de forma correcta.</p>
        <p>Tenga en cuenta que podemos llegar a tardar hasta 72 horas en responder. Hemos recibido la siguiente informacion de contacto</p>
        <hr>
        <p><b> Nombre: </b></p>
        <p><em>{{$details->name}}</em></p>
        <p><b> Email: </b></p>
        <p><em>{{$details->email}}</em></p>
        <p> <b> Sujeto: </b></p>
        <p><em>{{$details->subject}}</em></p>
        <p> <b> Mensaje: </b> </p>
        <em><p> {{$details->message}}</p>
        </em>
        <hr>
        <p>El numero de referencia de tu consulta es el siguiente <em><b>{{ $ref }}</b></em>, guarde este numero ya que puede ser util.</p>
